...
public class StdThursdayFragment extends Fragment {
    private RecyclerView recyclerView;
    private TimeTableListAdapter adapter;
    DatabaseReference mRef;
    FirebaseUser user;

    public StdThursdayFragment() {
    }
    ...
    public void onStart() {
        ...
        if(adapter == null){
            adapter = new TimeTableListAdapter(new ArrayList<TimeTableInfo>());
            recyclerView.setAdapter(adapter);
            mRef.child("coursematerials").child(AllcoursesFragment.stdCourseSelected).child("timetable").child("Thursday").addChildEventListener(new ChildEventListener() {
                @Override
