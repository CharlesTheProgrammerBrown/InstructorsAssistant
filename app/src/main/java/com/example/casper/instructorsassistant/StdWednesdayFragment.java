...
public class StdWednesdayFragment extends Fragment {
    private RecyclerView recyclerView;
    private TimeTableListAdapter adapter;
    DatabaseReference mRef;
    FirebaseUser user;

    public StdWednesdayFragment() {
    }
    ...
    public void onStart() {
        ...
        if(adapter == null){
            adapter = new TimeTableListAdapter(new ArrayList<TimeTableInfo>());
            recyclerView.setAdapter(adapter);
            mRef.child("coursematerials").child(AllcoursesFragment.stdCourseSelected).child("timetable").child("Wednesday").addChildEventListener(new ChildEventListener() {
                @Override
